...
export class Login {
    ...
    static async toSfdc (page: Page): Promise<void> {
        await page.goto(`https://login.salesforce.com?un=${this.username}&pw=${this.password}`, {waitUntil: "domcontentloaded"});
        if (page.url.toString().includes("LoginInterstitial")){
