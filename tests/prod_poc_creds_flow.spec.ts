...
import { BASIC_CREDENTIALS } from "../sfdx-auth/tools/UsernamePasswordFetcher";
...
test.beforeAll(async () => {
    const credentials:BASIC_CREDENTIALS = JSON.parse((await readFile('sfdx-auth/credentials-flow/credentials.json')).toString());
    api = await new SFDC_API({username: credentials.sandboxes[0].users[1].credentials.username, password: credentials.sandboxes[0].users[1].credentials.password}).Ready;
    ui = await new SFDC_UI(LoginFlow.Credentials).Ready;
})

test.describe.parallel('SFDC-prod-poc-creds-flow', () => {
